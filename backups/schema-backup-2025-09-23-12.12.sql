...
CREATE TABLE public.positions (
    ...
    CONSTRAINT positions_option_type_check CHECK ((option_type = ANY (ARRAY['CALL'::text, 'PUT'::text])))
);
...
CREATE TABLE public.transactions (
    ...
    CONSTRAINT transactions_transaction_type_check CHECK ((transaction_type = ANY (ARRAY['dividend'::text, 'interest'::text, 'buy'::text, 'sell'::text, 'transfer_in'::text, 'transfer_out'::text, 'fee'::text, 'return_of_capital'::text, 'assignment'::text, 'redemption'::text, 'reinvest'::text, 'option_buy'::text, 'option_sell'::text, 'other'::text])))
);
